...
df3 = olympics_top20 %>%
  ...
  select(name, age, sex, year)
df3

df3 = df3 %>%
  mutate(agef = ifelse(df3$sex == "F", df3$age, 0)) %>%
  mutate(agem = ifelse(df3$sex == "M", df3$age, 0))
df3

agevariation = df3 %>%
  group_by(year) %>%
  summarise(avgall = mean(age, na.rm = TRUE), avgf = mean(agef, na.rm = TRUE), avgm = mean(agem, na.rm = TRUE))
agevariation

view(agevariation)
  
ggplot(agevariation)+
  geom_line(aes(x = year, y = avgall))+
  geom_line(aes(x = year, y = avgm, color = 'green'))+
  geom_line(aes(x = year, y = avgf, color = 'red'))+
  labs(x = "Years when Swimming event was held at Olympics", y = "Average Age of Swimmers", caption = "")
